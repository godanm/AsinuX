...
class FirebaseService {
  ...
  Future<void> _resolveTrick(
    ...
    // For non-cut players we trust Firebase (their hands were written individually
    // when they played), so read fresh counts.
    final freshPlayersSnap =
        await _roomRef(state.roomId).child('players').get();
    final freshPlayerMap = <String, Player>{};
    if (freshPlayersSnap.exists) {
      final raw = freshPlayersSnap.value as Map<dynamic, dynamic>;
      raw.forEach((k, v) {
        final id = k.toString();
        freshPlayerMap[id] =
